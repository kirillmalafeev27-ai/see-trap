...
function centerMarker() {
  return { r: Math.floor(state.size / 2), c: Math.floor(state.size / 2) };
}
...
io.on("connection", (socket) => {
  ...
  socket.on("teacher:shuffleBoardWords", () => {
    ...
    if (state.phase !== PHASE.SETUP_WORDS) {
      sendError(socket, "Перемешивание поля доступно только на этапе подготовки.");
      return;
    }
  ...
  socket.on("teacher:finishWordPlacement", () => {
    ...
    state.phase = PHASE.PLAYER_MOVE;
    state.info = "Игра запущена. Ход игрока.";
    emitState();
  });
  ...
  socket.on("game:restart", () => {
    ...
    if (state.phase === PHASE.PLAYER_MOVE) {
      state.info = `${actor} перезапустил игру. Ход игрока.`;
    } else {
  ...
  socket.on("game:cellClick", ({ r, c, wordId }) => {
    ...
    if (state.phase === PHASE.PLAYER_BLOCK && state.currentTurn === "player") {
      ...
      state.requiredWords = state.lastPathWords.slice();
      state.sentenceText = "";
      state.sentenceSubmitted = false;
